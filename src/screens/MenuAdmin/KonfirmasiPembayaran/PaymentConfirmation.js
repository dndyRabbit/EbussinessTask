...
const PaymentConfirmation = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    setDeleted(false);
    return fetchPaymentUser();
  }, [loading, navigation, deleted]);

  //ambil data payment evidence di firestore
  ...
  const deleteOrderOngoing = async ({...props}) => {
    console.log(props.name);
    await firestore()
      .collection('order_ongoing')
      .doc(`${props.email}.${props.orderId}`)
              ...
              <View
                ...
                <View
                  ...
                  <Text
                    ...
                    Time Order: {moment(item.uploadedTime.toDate()).fromNow()}
                  </Text>
                  <Text
                    style={{
                      color: '#fff',
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        storeFailedOrder({
                          ...
                          orderStatus: 'Pembayaran gagal',
                          outlet: item.outlet,
                          time: item.uploadedTime,
                          totalItem: item.itemQty,
                          userId: item.userId,
